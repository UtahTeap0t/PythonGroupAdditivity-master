...
class GroupLibrary(Mapping):
    ...
    *property set* `name` and  set of groups contained in the chemical
    ...
    def Estimate(self, groups, property_set_name):
        ...
        active_groups = [group for group in groups
                          if property_set_name in self[group]]
        print("these groups are ok: ", active_groups)
        if missing_groups:
            print("a group error happened at: " , missing_groups)
